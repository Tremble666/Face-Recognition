...
@WebServlet("/facerecognition/drawFace")
public class drawFaceServlet extends HttpServlet {
	...
	private void useSub(HttpServletRequest req, HttpServletResponse resp)
		...
		if (user == null) {
			...
			req.getRequestDispatcher("/WEB-INF/views/drawFace.jsp")
					.forward(req, resp);
		}
	}
	...
	private void handleSub(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			//运行python脚本将脸圈出来
			fileutil.runPython("D:\\APP_Project\\java_4_ee_kepler\\FaceRecognize\\webapp\\python\\draw.py");
			
			//拿到识别信息后新型判断，分为两种情况
			req.setAttribute("resultUrl", "/drawFace/result.jpg");
			
			req.setAttribute("imgUrl", "/drawFace" + "/" + "1.jpg");
			req.getRequestDispatcher("/WEB-INF/views/drawFace.jsp")
					.forward(req, resp);
